...
// Check if SD card is available by testing if directory exists
static bool is_sdcard_available() {
    ...
    if (stat("/sdcard", &st) == 0 && S_ISDIR(st.st_mode)) {
        ...
        if (stat(SDCARD_PRINTER_PATH, &st) != 0) {
            // Create the printer directory
            if (mkdir(SDCARD_PRINTER_PATH, 0755) == 0) {
                ESP_LOGI(TAG, "Created printer directory on SD card: %s", SDCARD_PRINTER_PATH);
            } else {
                ESP_LOGW(TAG, "Failed to create printer directory: %s (errno=%d)", SDCARD_PRINTER_PATH, errno);
                // Check if it exists anyway (race condition)
                if (stat(SDCARD_PRINTER_PATH, &st) != 0) {
                    return false;  // Directory doesn't exist and couldn't create
                }
            }
        }
        return true;
...
// Get the appropriate printer cache path
            ...
            if (event->data_len > 0 && event->data_len <= 20480) {
                ...
                if (json_str) {
                    ...
                    if (json_data) {
                        ...
                        // Write MINIMAL printer data to SPIFFS cache
                        // Only extract essential fields to save space (~500 bytes vs 7KB+)
                        if (serial[0] != '\0') {  // Only write if we have serial number
                            ...
                            if (tv_now.tv_sec - last_write_time >= 5) {
                                // Create minimal JSON with only essential display fields
                                cJSON *mini_json = cJSON_CreateObject();
                                if (mini_json) {
                                    // Timestamp for online detection
                                    cJSON_AddNumberToObject(mini_json, "last_update", tv_now.tv_sec);
                                    
                                    // State (gcode_state field)
                                    cJSON *gcode_state = cJSON_GetObjectItem(json_data, "gcode_state");
                                    if (gcode_state && gcode_state->valuestring) {
                                        cJSON_AddStringToObject(mini_json, "state", gcode_state->valuestring);
                                    } else {
                                        cJSON_AddStringToObject(mini_json, "state", "IDLE");
                                    }
                                    
                                    // Progress percentage
                                    cJSON *mc_percent = cJSON_GetObjectItem(json_data, "mc_percent");
                                    cJSON_AddNumberToObject(mini_json, "progress", 
                                        mc_percent ? mc_percent->valueint : 0);
                                    
                                    // Remaining time (minutes)
                                    cJSON *mc_remaining = cJSON_GetObjectItem(json_data, "mc_remaining_time");
                                    cJSON_AddNumberToObject(mini_json, "remaining_min",
                                        mc_remaining ? mc_remaining->valueint : 0);
                                    
                                    // Layer info
                                    cJSON *layer_num = cJSON_GetObjectItem(json_data, "layer_num");
                                    cJSON *total_layers = cJSON_GetObjectItem(json_data, "total_layer_num");
                                    cJSON_AddNumberToObject(mini_json, "current_layer",
                                        layer_num ? layer_num->valueint : 0);
                                    cJSON_AddNumberToObject(mini_json, "total_layers",
                                        total_layers ? total_layers->valueint : 0);
                                    
                                    // Temperatures
                                    cJSON_AddNumberToObject(mini_json, "nozzle_temp",
                                        nozzle_temp ? nozzle_temp->valuedouble : 0);
                                    cJSON *nozzle_target = cJSON_GetObjectItem(json_data, "nozzle_target_temper");
                                    cJSON_AddNumberToObject(mini_json, "nozzle_target",
                                        nozzle_target ? nozzle_target->valuedouble : 0);
                                    cJSON_AddNumberToObject(mini_json, "bed_temp",
                                        bed_temp ? bed_temp->valuedouble : 0);
                                    cJSON *bed_target = cJSON_GetObjectItem(json_data, "bed_target_temper");
                                    cJSON_AddNumberToObject(mini_json, "bed_target",
                                        bed_target ? bed_target->valuedouble : 0);
                                    
                                    // Fan speeds (0-15 scale)
                                    cJSON *cooling_fan = cJSON_GetObjectItem(json_data, "cooling_fan_speed");
                                    cJSON_AddNumberToObject(mini_json, "part_fan",
                                        cooling_fan ? atoi(cooling_fan->valuestring) : 0);
                                    
                                    // Print job info
                                    cJSON *subtask = cJSON_GetObjectItem(json_data, "subtask_name");
                                    if (subtask && subtask->valuestring) {
                                        cJSON_AddStringToObject(mini_json, "subtask_name", subtask->valuestring);
                                    }
                                    cJSON *gcode_file = cJSON_GetObjectItem(json_data, "gcode_file");
                                    if (gcode_file && gcode_file->valuestring) {
                                        // Extract just filename from path
                                        const char *filename = strrchr(gcode_file->valuestring, '/');
                                        cJSON_AddStringToObject(mini_json, "file_name", 
                                            filename ? filename + 1 : gcode_file->valuestring);
                                    }
                                    
                                    // WiFi signal
                                    cJSON *wifi = cJSON_GetObjectItem(json_data, "wifi_signal");
                                    if (wifi && wifi->valuestring) {
                                        cJSON_AddStringToObject(mini_json, "wifi_signal", wifi->valuestring);
                                    }
                                    
                                    // Convert minimal JSON to string
                                    char* json_output = cJSON_PrintUnformatted(mini_json);
                                    if (json_output) {
                                        std::string filename = get_printer_cache_path(serial);
                                        ESP_LOGD(TAG, "Writing printer cache to: %s", filename.c_str());
                                        FILE* printer_file = fopen(filename.c_str(), "w");
                                        if (printer_file) {
                                            size_t len = strlen(json_output);
                                            size_t written = fwrite(json_output, 1, len, printer_file);
                                            fflush(printer_file);
                                            fclose(printer_file);
                                            if (written == len) {
                                                ESP_LOGI(TAG, "Wrote printer cache: %s (%zu bytes)", filename.c_str(), written);
                                                last_write_time = tv_now.tv_sec;
                                            } else {
                                                ESP_LOGW(TAG, "Partial write to %s: %zu/%zu bytes", filename.c_str(), written, len);
                                            }
                                        } else {
                                            ESP_LOGW(TAG, "Failed to open printer cache: %s (errno=%d: %s)", 
                                                filename.c_str(), errno, strerror(errno));
                                            // Fallback to SPIFFS if SD card failed
                                            if (filename.find("/sdcard/") == 0) {
                                                std::string spiffs_path = std::string(SPIFFS_PRINTER_PATH) + "/" + serial + ".json";
                                                ESP_LOGI(TAG, "Trying fallback to SPIFFS: %s", spiffs_path.c_str());
                                                FILE* spiffs_file = fopen(spiffs_path.c_str(), "w");
                                                if (spiffs_file) {
                                                    size_t len = strlen(json_output);
                                                    size_t written = fwrite(json_output, 1, len, spiffs_file);
                                                    fflush(spiffs_file);
                                                    fclose(spiffs_file);
                                                    if (written == len) {
                                                        ESP_LOGI(TAG, "Wrote to SPIFFS fallback: %s (%zu bytes)", spiffs_path.c_str(), written);
                                                        last_write_time = tv_now.tv_sec;
                                                    }
                                                } else {
                                                    ESP_LOGE(TAG, "SPIFFS fallback also failed: %s (errno=%d)", spiffs_path.c_str(), errno);
                                                }
                                            }
                                        }
                                        cJSON_free(json_output);
                                    }
                                    cJSON_Delete(mini_json);
                                }
                            }
                        }
